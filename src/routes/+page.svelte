<script lang="ts">
    ...
    let image1: HTMLElement, image2: HTMLElement, image3: HTMLElement, image4: HTMLElement, image5: HTMLElement, image6: HTMLElement;

    let track_width = $state(0);
    let half_width = $derived(((track_width - 40 * 5) / 12));
    let click_x = $state(0);
...
<div role="button" tabindex="0" class="h-screen w-screen text-white overflow-visible" onmousemove={handle_mouse_move} onmousedown={handle_mouse_down} onmouseup={handle_mouse_up}>
    <div id="track" class="flex w-max gap-14 absolute left-1/2 top-1/2 pointer-events-none" style="transform: translate(-{half_width / track_width * 100}%, -50%)" bind:this={track} bind:clientWidth={track_width}>
        <img class="w-[40vmin] h-[56vmin] object-cover object-center" alt="image1" src="{img1}" style="object-position: {100 - (half_width / track_width * 100 + ((5 * half_width + 140) / track_width * 100))}% center" draggable="false" bind:this={image1} />
